  ...
  */
  ...
  deleteSelectedNodes() {
    ...
    while (this.data.meta.selected_nodes.length > 0) {
      const node_index = this.data.meta.selected_nodes.pop();
      const node = this.data.nodes[node_index];
  ...
  deleteEdge(edge_index, deletedNode) {
    ...
    for (const i in this.data.edges) {
  ...
  selectEdge(event, edge_index) {
    ...
    if (event === 'ctrl') {
      ...
      if (this.selectedEdge.length > 0) {
        for (const e of this.selectedEdge) { this.data.edges[e].selected = false; }
      }
  ...
  focusFlowchart(): void {
    ...
    const frame = [this.data.nodes[0].position.x, this.data.nodes[0].position.y,
