...
export class GIFuncsMake {
    ...
    private _sweepPosis( backbone_wire_i: number, xsection_wire_i: number, divisions: number): number[][] {
        ...
        const xsextion_xyzs: Txyz[] = this.modeldata.attribs.query.getEntCoords(EEntType.WIRE, xsection_wire_i);
        ...
        const wire_xyzs: Txyz[] = this.modeldata.attribs.query.getEntCoords(EEntType.WIRE, backbone_wire_i);
        let plane_xyzs: Txyz[] = [];
    ...
    private _cutCopyEnt(ent_type: EEntType, ent_i: number, exist_posis_i: number[], posi_to_copies: number[]): number {
        ...
        switch (ent_type) {
            case EEntType.PLINE:
                ...
                this.modeldata.geom.modify.replacePosis(ent_type, new_pline_i, new_posis_i);
                return new_pline_i;
            case EEntType.PGON:
                const new_pgon_i: number = this.modeldata.geom.add.copyPgons(ent_i, true) as number;
                this.modeldata.geom.modify.replacePosis(ent_type, new_pgon_i, new_posis_i);
                return new_pgon_i;
            default:
                break;
        }
