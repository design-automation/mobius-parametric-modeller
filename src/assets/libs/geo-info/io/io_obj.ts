 ...
 * Export to obj
 */
export function exportVertBasedObj(model: GIModel, entities: TEntTypeIdx[], ssid: number): string {
    ...
    const [pgons_i, plines_i]: [number[], number[]] = _getPgonsPlines(model, entities, ssid);
 ...
 * Export to obj
 */
export function exportPosiBasedObj(model: GIModel, entities: TEntTypeIdx[], ssid: number): string {
    const h_str = '# File generated by Mobius.\n';
    // the order of data is 1) vertex, 2) texture, 3) normal
    let v_str = '';
    let f_str = '';
 ...
 */
function _getPgonsPlines(model: GIModel, entities: TEntTypeIdx[], ssid: number): [number[], number[]] {
    let pgons_i: number[] = [];
    let plines_i: number[] = [];
    if (entities === null) {
