...
import { IGeomMaps, TTri, TEdge, TWire, TFace,
    TVert, TFaceTri, TPoint, TPline, TPgon, EEntType, IEntSets, IGeomJSONData, TEntTypeIdx } from './common';
...
export class GIGeomImpExp {
    ...
    public importGI( gi_data: IGeomJSONData ): Map<EEntType, Map<number, number>> {
        ...
        for (const other_pline_i of gi_data.plines_i) {
            renum_plines_map.set(other_pline_i, this._geom.modeldata.model.metadata.nextPline());
        }
        // pgons
        ...
        for (let i = 0; i < gi_data.posis_i.length; i++) {
            ...
            // snapshot
            this._geom.snapshot.addEntActive(EEntType.POSI, other_posi_i);
        }
        ...
        for (let i = 0; i < gi_data.points_i.length; i++) {
            ...
            // timestamp
            this._geom.modeldata.updateEntTs(EEntType.POINT, other_point_i);
            // snapshot
            this._geom.snapshot.addEntActive(EEntType.POINT, other_point_i);
        }
        // add plines to model
        for (let i = 0; i < gi_data.plines_i.length; i++) {
            ...
            // timestamp
            this._geom.modeldata.updateEntTs(EEntType.PLINE, other_pline_i);
            // snapshot
            this._geom.snapshot.addEntActive(EEntType.PLINE, other_pline_i);
        }
        // add pgons to model
        for (let i = 0; i < gi_data.pgons_i.length; i++) {
            ...
            // timestamp
            this._geom.modeldata.updateEntTs(EEntType.PGON, other_pgon_i);
            // snapshot
            this._geom.snapshot.addEntActive(EEntType.PGON, other_pgon_i);
        }
        // add colls to model
        for (let i = 0; i < gi_data.colls_i.length; i++) {
            ...
            // snapshot
            this._geom.snapshot.addEntActive(EEntType.COLL, other_coll_i);
        }
        // ========================================================================================
        // return the maps
        return renum_maps;
