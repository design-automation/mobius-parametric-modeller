...
export class GIModel {
    ...
    public get3jsData(): IThreeJS {
        ...
        const [posis_xyz, posis_map]: [number[], Map<number, number>]  =  this.attribs.threejs.get3jsSeqPosisCoords();
        const [vertex_xyz, vertex_map]: [number[], Map<number, number>]  =  this.attribs.threejs.get3jsSeqVertsCoords();
        const normals_values: number[] = this.attribs.threejs.get3jsSeqVertsAttrib(EAttribNames.NORMAL);
        ...
        const [tris_verts_i, triangle_select_map]: [number[], Map<number, number>] = this.geom.threejs.get3jsTris(vertex_map);
        const [edges_verts_i, edge_select_map]: [number[], Map<number, number>] = this.geom.threejs.get3jsEdges(vertex_map);
        const [points_verts_i, point_select_map]: [number[], Map<number, number>] = this.geom.threejs.get3jsPoints(vertex_map);
