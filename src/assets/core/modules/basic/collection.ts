...
export function Add(__model__: GIModel, coll: TId, entities: TId|TId[]): void {
    ...
    if (!isEmptyArr(entities)) {
        ...
        let ents_arr: TEntTypeIdx[];
        if (__model__.debug) {
            coll_arr = checkIDs(fn_name, 'coll', coll, [IDcheckObj.isID], [EEntType.COLL]) as TEntTypeIdx;
            ents_arr = checkIDs(fn_name, 'entities', entities,
...
export function Delete(__model__: GIModel, coll: TId|TId[]): void {
    ...
    let colls_arrs;
    if (__model__.debug) {
        colls_arrs = checkIDs(fn_name, 'coll', coll, [IDcheckObj.isIDList], [EEntType.COLL]) as TEntTypeIdx[];
    } else {
